var tree = require('nebbula-tree');
var nerror = require('nebbula-error');
var parser = require('nebbula-parser');
var Node = exports;
...
Node.toListNode = function(node) {
	...
	case 'argumentList':
		...
		return {
			type: 'list',
			children: tree(node.children, sep),
			loc: node.loc,
		};
	}
...
Node.perform = function (op, left, right) {
	...
	case 'dimension / number':
		...
		if (divisor === 0) throw new nerror("Divide by zero", right);
		var clone = Node.clone(left);
	...
	case 'dimension % number':
		var divisor = right.children[0];
		if (divisor === 0) throw new nerror("Modulo by zero", right);
		var clone = Node.clone(left);
		clone.children[0] %= right.children[0];
		return clone;
	case 'number % percentage':
	...
	throw new nerror("Unsupported binary operation: " + left.type + ' ' + op + ' ' + right.type, left);
};
